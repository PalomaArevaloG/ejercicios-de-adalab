...
const linkDropdown = document.querySelector(".js_link_dropdown"); //l18html
...
// Día 5: Se escucha el click en el enlace del botón hamburguesa l18html
// para mostrar u ocultar el menú dropdown.

function handleClickLinkDropdown(event) {
    event.preventDefault(); //prevenimos que se recargue la pagina al ser un enlace el elto al poner el event en l215 js y esta linea. debe ir en la primera linea dee la funcion

    toggleDropDownMenu(); //va con parentesis para ejecutar la funcion de l187js
}

linkDropdown.addEventListener("click", handleClickLinkDropdown); //no se ponen los parentesis en la funcion
...
buttonAdd.addEventListener("click", showAddForm); //al no necesitar el event.preventdefault por no estar en un formulario lo podemos hacer sin función handle
